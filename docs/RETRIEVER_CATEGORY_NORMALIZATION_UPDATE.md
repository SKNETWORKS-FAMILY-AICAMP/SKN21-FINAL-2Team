# Retriever/필터/성능 개선 업데이트 (2026-03-03)
...
- `PlaceRetriever.normalize_category()`를 추가하여 입력 카테고리를 `contenttypeid` 표준값으로 정규화한다.
...
- `retrieval_place()`의 결과 포맷팅에서 사진 조회 N+1 쿼리를 제거하고, `contentid` 목록 배치 조회로 변경한다.
- `backend/app/agents/models/output.py`의 planner category 스키마 설명을 `contenttypeid` 표준 6개 값으로 통일한다.
- `backend/app/agents/retriever.py`에서 미사용 import를 제거한다.
- 대표 매핑:
  - `숙소` -> `숙박`
  - `카페` -> `음식점`
  - `체험` -> `레포츠`
...
- 검증 항목:
  ...
  - 사진 URL 배치 조회 결과의 contentid별 그룹핑/중복제거 검증
