...
export function MomentGallery({
  ...
  return (
    <div className="columns-1 gap-4 space-y-4 md:columns-2 xl:columns-3 2xl:columns-4">
      {diaries.map((moment) => {
        const isChecked = selectedToDelete.has(moment.id);
        return (
          <button
            key={moment.id}
            onClick={() => {
              if (isDeleteMode) {
                onToggleDeleteSelect?.(moment.id);
              } else {
                onSelect(moment.id);
              }
            }}
          ...
          >
            <img
              src={moment.image_path || DEFAULT_PLACEHOLDER}
              alt={moment.title}
            ...
            {/* 선택된 카드 테두리 (일반 모드) */}
            {!isDeleteMode && moment.id === selectedDiaryId && (
            ...
            <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/85 via-black/35 to-transparent p-4 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
              ...
              <p className="line-clamp-1 text-lg font-medium text-white">{moment.title}</p>
              {moment.adress && (
                <p className="mt-1 flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-white">
                  <MapPin size={10} /> {moment.adress}
                </p>
              )}
            </div>
          </button>
