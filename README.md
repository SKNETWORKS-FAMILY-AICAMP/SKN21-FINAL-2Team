...
### 문제 1
**LLM 에이전트 응답 지연(Latency)연쇄적인 노드 호출과 무거운 검색 로직으로 인해 사용자가 체감하는 첫 응답 시간(TTFT)과 전체 생성 시간이 길어지는 문제 발생**
- 해결 방법: 평균 레이턴시 축소 6대 전략 적용
  1. Reranker 모델 경량화 도입
  2. Intent 노드의 분석 작업 분리 및 비동기 병렬 처리 적용
  3. Multi-channel Retriever 병렬 검색 구현
  4. 불필요한 Candidates 개수 과감히 축소 (20개 → 5개)
  5. Executor 노드 계획 추천 시 가장 적합한 Top 1개 후보군만 사용하도록 간소화
  6. 프롬프트 엔지니어링을 통한 불필요 출력 토큰 최적화                                                                                                                                                        - 수행 결과: 각 에이전트 노드의 병목 현상이 해소되어 **전체 파이프라인의 평균 레이턴시가 대폭 감소**하였으며, 부드럽고 빠른 실시간 대화 스트리밍 체감 속도 확보
### 문제 3) 장소 맥락 인식 불가 (키워드 매칭 오류)
**'강남에서 갈 수 있는 카페' 요청 시, 실제 강남이 아닌 이름만 똑같은 '홍대에 있는 강남카페'가 검색되는 등 공간적(Spatial) 맥락 인지 실패 문제 발생**
- 해결 방법: 3단계 공간 지능화(Spatial Intelligence) 로직 도입:
  1. LLM 의도 분석 좌표 추출:** 사용자 발화('삼성역 조용한 카페')에서 지명 의도를 분리해 중심 좌표(위경도)를 선제적으로 도출**
  2. 로컬 지역 사전(Landmark Dictionary) 구축:** '송리단길', '샤로수길' 등 행정구역에 없는 핫플레이스 고유명사에 위경도와 반경(예: 800m)을 하드 매핑하여 해결**
  3. 동적 Geocoding 연결 (Naver API):** 사전에 없는 신규 장소('엔코아미술관')가 요청되었을 경우, `Naver Search API`로 실제 지번 주소를 동적 추출 후 `Naver GeoCoder`를 태워 좌표로 번역하는 강력한 Fallback 회로 구축
- 수행 결과: 단순 텍스트 표기 매칭으로 일어나는 엉뚱한 오안내(False Positive)가 완벽히 차단됨. 사용자의 요청 위치를 기반으로 한 **정확한 반경 기반(Geo Filtering)** 매칭이 이루어져 추천 신뢰도 극대화
...
| 팀원   | 회고 내용 |
| :----- | :-------- |
| 전우영 | 팀 Trivers의 PM으로, 프로젝트 기획부터 개발, 배포, 마무리까지 전체 과정을 핸들링했습니다. 그 중에서도 데이터 파트와 프론트엔드 영역을 중점적으로 작업했는데요. 추천 장소 DB를 구축하기 위해 국내 여행지를 긁어 모으는 과정에서는 데이터 서칭 역량을, 데이터를 적재하고 관리하는 과정에서는 VectorDB(Qdrant)와 RDB(MySQL)에 대한 이해도를 높였습니다. 좋은 서비스는 질 좋은 데이터로부터 나오는 것임을 뼈저리게 느꼈습니다. 프론트엔드 영역에서는 서비스의 화면을 설계하고 구현했는데요. 특히 초기 설계 단계에서 화면 view를 손으로 직접 그려보았던 게 기억에 남습니다. 사용자가 우리의 서비스를 더 효과적으로 이용하려면 어떤 기능을 어느 영역에 어떻게 배치해야할지 고민할 수 있었거든요. 작은 버튼이라도 그 위치에는 다 이유가 있음을 느꼈습니다.<br>무엇보다 파이널 프로젝트에서 얻은 가장 큰 소득은 PM 경험이 아닐까 싶은데요. 저는 PM으로써 팀 목표 달성을 위해 계획을 수립하고, 진행률을 체크하고, 팀원이 개인의 역량을 발휘할 방법을 찾는 데 힘을 쏟았습니다. 이 과정에서 PM이라고 해서 모든 영역을 커버해야 하는 것이 아니고, 그렇게 할 수도 없기 때문에, 팀원들과 상호보완하며 나아가야 한다는 것도 깨달았습니다. <br> 한 가지 아쉬운 점이 있다면, PM으로 역할하면서 동시에 백엔드 서버 영역, LLM 영역까지 깊게 경험하기에는 한계가 있었다는 것입니다. 조금 더 개발 쪽을 진하게 경험해봤다면 좋았을 텐데 하는 아쉬움이 남습니다. 다만, 그렇다 보니 전공자이자 현직자로서 가람님이 백엔드 및 모델링 영역 (사실 전체)에서 원맨쇼를 펼칠 수밖에 없었는데요. 이 자리를 빌어 감사와 경의을 표합니다. 더불어 비전공자로서 이 과정 자체가 쉽지 않았을 텐데도 불구하고, 맡은 일에 최선을 다해주시고 역량을 최대치로 끌어올리며 함께 완주해준 민정님과 현우님에게도 고마움을 전합니다. (포기하지 않는 인간은 정말 멋져) 또, 저희가 방향을 잃지 않고 잘 달려갈 수 있도록 진심을 다해 이끌어주신 아이디어 뱅크이자, 이정표이자, 채찍이자, 페이스 메이커이셨던 멘토님께도 감사인사 전합니다. 진짜 마지막으로!!! 앞선 4개월 동안 기초부터 탄탄하게 잘 가르쳐주신 강사님과 엉망출석 21기를 관리해주신 매니저님들께도 감사드립니다. 끝!|
